...
class TestWellness(unittest.TestCase):
    ...
    def test_mood_weather_sunny(self):
            self.assertEqual(Wellness.mood_weather("sunny"),"The sun is shining out! Take advantage of this beautiful sunshine and soak up some vitamin D (not too much though, wear your sunscreen).") 
    
    def test_mood_weather_not_sunny(self):
            self.assertNotEqual(Wellness.mood_weather("rainy"),"The sun is shining out! Take advantage of this beautiful sunshine and soak up some vitamin D (not too much though, wear your sunscreen).")    
    
    def test_mood_weather_cloudy(self):
            self.assertEqual(Wellness.mood_weather("cloudy"),"There is no sun today. You might be feeling a little down today, but don't let that stop you from having a great day.")
    
    def test_mood_weather_not_cloudy(self):
            self.assertNotEqual(Wellness.mood_weather("sunny"),"There is no sun today. You might be feeling a little down today, but don't let that stop you from having a great day.")    
    
    def test_mood_weather_rainy(self):
            self.assertEqual(Wellness.mood_weather("rainy"),"It's a rainy one today. Try to keep your spirits high even though today might seem dreary.")
    
    def test_mood_weather_not_rainy(self):
            self.assertNotEqual(Wellness.mood_weather("snowy"),"It's a rainy one today. Try to keep your spirits high even though today might seem dreary.")    
    
    def test_mood_weather_snow(self):
            self.assertEqual(Wellness.mood_weather("snowy"),"It's snowing outside. Snow can really bring down our mood sometimes, so it is important to take care of yourself today.")
    
    def test_mood_weather_not_snowy(self):
            self.assertNotEqual(Wellness.mood_weather("rainy"),"It's snowing outside. Snow can really bring down our mood sometimes, so it is important to take care of yourself today.")   
    
    def test_mood_weather_thunderstorms(self):
            self.assertEqual(Wellness.mood_weather("thunderstorms"),"It's storming out. If you're up for it, watch the storms roll through. Otherwise, take care of yourself because sunny days are inevitable.")
    
    def test_mood_weather_not_thunderstorms(self):
        self.assertNotEqual(Wellness.mood_weather("sunny"),"It's storming out. If you're up for it, watch the storms roll through. Otherwise, take care of yourself because sunny days are inevitable.")   
    
    def test_mood_weather_not_valid_partly_sunny(self):
            self.assertEqual(Wellness.mood_weather("partly sunny"),"Weather condition not recognized. Please enter a valid condition (sunny, cloudy, snowy, rain, or thunderstorm).")
    
    def test_mood_weather_not_valid_ice(self):
            self.assertEqual(Wellness.mood_weather("ice"),"Weather condition not recognized. Please enter a valid condition (sunny, cloudy, snowy, rain, or thunderstorm).")
